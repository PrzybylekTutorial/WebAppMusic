...
const DynamicPlaylistManager: React.FC<DynamicPlaylistManagerProps> = ({ accessToken, onPlaylistCreated, onPlaylistUpdate }) => {
  ...
  const applyFilters = useCallback(
    (list: any[]) => {
      const genreFilter = searchFilters.genre?.trim().toLowerCase();
      const artistFilter = searchFilters.artist?.trim().toLowerCase();

      return list.filter((song: any) => {
        const songGenre = (song.genre || '').toLowerCase();
        const songArtist = (song.artist || '').toLowerCase();

        const matchGenre = genreFilter ? songGenre.includes(genreFilter) : true;
        const matchArtist = artistFilter ? songArtist.includes(artistFilter) : true;
  ...
  const searchSongs = useCallback(async (query: string) => {
    ...
    try {
      ...
      // Deduplicate by title + artist to avoid repeated suggestions
      ...
      setSearchResults(applyFilters(deduped));
    } catch (error) {
  ...
  // Re-apply filters whenever filters or raw results change
  useEffect(() => {
    if (rawResults.length > 0) {
      setSearchResults(applyFilters(rawResults));
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [rawResults, searchFilters.genre, searchFilters.artist, applyFilters]);
